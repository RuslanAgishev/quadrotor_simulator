function [t_out, s_out, d_state] = simulation_2d(controlhandle, trajhandle)
...
if video
  video_writer = VideoWriter('traj_2d.avi', 'Uncompressed AVI');
  open(video_writer);
end
h_fig = figure;
...
%% *********************** INITIAL CONDITIONS ***********************
t_total  = params.timelim;% Total simulated time
...
pos_tol = params.postol;
vel_tol = params.veltol;
ang_tol = params.angtol;
...
for iter = 1:max_iter
  ...
  if iter == 1
    ...
    QP = QuadPlot(1, quad_state, params.cross.length, 0, quadcolors(1,:), max_iter, h_3d);
...
disp(['Position accuracy   [m]: ', num2str(norm(x(1:2) - stop_pos))]);
disp(['Velocity accuracy [m/s]: ', num2str(norm(x(4:5)))]);
disp(['Final angle       [rad]: ', num2str(abs(x(3)))]);
end
