...
%
% F = u1 + u2;
% M = (u2 - u1)*params.cross.length;

u1 = 0.5*(F - M/params.cross.length);
u2 = 0.5*(F + M/params.cross.length);
...
M_clamped = (u2_clamped - u1_clamped)*params.cross.length;
...
sdot = [s(4);
        s(5);
        s(6);
        Wy/params.mass - F_clamped*sin(s(3))/params.mass;
        Wz/params.mass + F_clamped*cos(s(3))/params.mass - params.gravity;
